/**
 * A large portion of this file was generated by AI (Cursor). However, there were
 * a lot of mistakes, and it seemed to struggle with the fact that my schemas were
 * separated into many different files for organization. It would often use fields
 * from one schema in another, which required manual fixing.
 * 
 * Example prompt (while giving it access to all files in schema folder):
 *  - Generate a seed file that populates all the tables with data, testing every feature in the app, including all 4 event series types.
 */
